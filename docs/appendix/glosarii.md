...
### Ability
This is a general term referring to rules that provide an exception to the basic rules. An ability could come from a number of sources, so âan ability that gives you a bonus to damage rollsâ could be a feat, a spell, and so on.
### Ability Boost
...
### Ability Score
Each creature has six ability scores: Strength, Dexterity, Constitution, Intelligence, Wisdom, and Charisma. These scores represent a creatureâs raw potential and basic attributes.
### Abjuration (trait)
...
### Agile (weapon trait)
The multiple attack penalty you take with this weapon on the second attack on your turn is â4 instead of â5, and â8 instead of â10 on the third and subsequent attacks in the turn.
### Aid [reaction] (basic action)
Reaction used to Grant a bonus to an allyâs skill check or attack roll.
### Air (trait)
...
### Alchemical (trait)
Alchemical items are powered by reactions of alchemical reagents. Alchemical items arenât magical and donât radiate a magical aura.
### Alchemist (trait)
This indicates abilities from the alchemist class.
### Alignment
Alignment represents a creatureâs basic moral and ethical attitude on a law-to-chaos axis and a good-to-evil axis.
...
### Animal (trait)
An animal is a creature with a relatively low intelligence. It typically doesnât have an Intelligence ability modifier over â4, canât speak languages, and canât be trained in Intelligence -based skills.
### Apex (trait)
When you Invest an Item that has the apex trait, it improves one of your ability scores, either increasing it by 2 or to a total of 18, whichever grants the higher score. This gives you all the benefits of the new ability score until the investiture ends. An apex item grants this benefit only the first time itâs invested within a 24-hour period, and you can benefit from only one apex item at a time. If you attempt to invest an apex item when you already have one invested, you donât gain the ability score increase, though you do gain any other effects of Investing the Item.
### Arcana (skill)
...
### Armor Class (AC)
This score represents how hard it is to hit and damage a creature.Â It typically serves as the DC to hit a creature with an attack. AC = 10 + Dex modifier (up to your armorâs Dex Cap) + proficiency bonus + armorâs item bonus to AC + other bonuses + penalties.
...
### Aid
A reaction used to grant a bonus to an allyâs attack roll.
### Multiple attack Penalty
â5 on your second attack, â10 on further attacks
### Unarmed attack attack (trait)
An ability with this trait involves an attack. For each attack you make beyond the first on your turn, you take a multiple attack penalty.
### Attack of Opportunity [reaction]
A reaction fighters and some other characters gain.
### Auditory (trait)
Auditory actions and effects rely on sound. An action with the auditory trait can be successfully performed only if the creature using the action can speak or otherwise produce the required sounds. A spell or effect with the auditory trait has its effect only if the target can hear it. This applies only to sound-based parts of the effect, as determined by the GM. This is different from a sonic effect, which still affects targets who canât hear it (such as deaf targets) as long as the effect itself makes sound.
### Aura
...
### Beast (trait)
A creature similar to an animal but with an Intelligence modifier of â3 or higher is usually a beast. Unlike an animal, a beast might be able to speak and reason.
### Bleed (damage type)
A type of persistent damage.
### Blinded (condition)
Youâre unable to see.
...
### Broken (condition)
This item canât be used for its normal function until repaired.
### Broken Threshold (BT)
When an objectâs HP reaches this number, it becomes broken.
### Bulk
A value indicating an itemâs size, weight, and general awkwardness. It takes 10 items of light Bulk to equal 1 Bulk, and 1, coins are 1 Bulk. A character becomes encumbered if they carry Bulk greater than 5 + their Str modifier, and they canât carry more than 10 + their Str modifier.
### Bulwark (armor trait)
...
### Chaotic (trait)
Chaotic effects often manipulate energy from chaos-aligned Outer Planes and are anathema to lawful divine servants or divine servants of lawful deities. A creature with this trait is chaotic in alignment.Â Creatures can use abilities of an alignment without matching that alignment, but they might be anathema or change a creatureâs alignment over time with repeated use.
### Character
...
### Check
When you roll a d20 and add modifiers, bonuses, and penalties, then compare your result to a Difficulty Class, youâre attempting a check.
### Check Penalty
...
### Clumsy (condition)
You canât move as easily or gracefully as usual.
### Coerce (skill action)
...
### Common (trait)
Anything that doesnât list another rarity trait (uncommon, rare, or unique) automatically has the common trait. This rarity indicates that an ability, item, or spell is available to all players who meet the prerequisites for it.
### Companion (trait)
...
### Cone (area)
A cone shoots out from you in a quarter circle on the grid.Â When you aim a cone, the first square of that cone must share an edge with your space if youâre aiming orthogonally, or it must touch a corner of your space if youâre aiming diagonally. If youâre Large or larger, the first square can run along the edge of any square of your space. You canât aim a cone so that it overlaps your space. The cone extends out for a number of feet, widening as it goes, as shown in the Areas diagram. For instance, when a green dragon uses its breath weapon, it breathes a cone of poisonous gas thatoriginates at the edge of one square of its space and affects a quarter-circle area 30 feet on each edge.
If you make a cone originate from someone or something else, follow these same rules, with the first square of the cone using an edge or corner of that creature or objectâs space instead of your own.
### Confused (condition)
...
### Consumable (trait)
An item with this trait can be used only once. Unless stated otherwise, itâs destroyed after activation. Consumable items include alchemical items and magical consumables such as scrolls and talismans. When a character creates consumable items, they can make them in batches of four.
### Contact (trait)
...
### Cover
When youâre behind a physical obstacle, you get a +2 circumstance bonus to AC, Reflex saves vs. area effects, and Stealth checks. This increases to +4 for greater cover. Creatures can provide lesser cover: a +1 circumstance bonus to AC.
...
### Critical
You can get a greater successâa critical successâby rolling 10 above your DC or a worse failureâa critical failureâby rolling 10 lower than your DC.
### Critical hit (Strike)
...
### Cursebound (trait)
Spells with this trait increase the severity of your oracular curse when cast. Usually only revelation spells have this trait. You canât cast a cursebound spell if you donât have an oracular curse.
### d4, d6, d8, d10, d12, d20, and d%
Notations for different sizes of dice. âd20â is a twenty-sided die, for example.
### Daily preparations
During your morning preparations, you ready your gear, prepare spells, and otherwise get ready for your adventuring day.
### Damage
Damage dealt to a creature reduces that creatureâs Hit Points on a 1-to-1 basis. Melee damage roll = damage die of the weapon or unarmed attack + Str modifier + bonuses + penalties; Ranged damage roll = damage die of the weapon + Str modifier for a thrown weapon + bonuses + penalties.
### Massive damage
...
### Deception (skill)
Trick and mislead others. (Cha) â,
...
7, 8,
### Die roll
Any time you roll the dice, youâre making a roll.
...
### Diplomacy (skill)
Influence others through negotiation and flattery. (Cha) â,
...
### Effect
An effect is the result of an ability, though an abilityâs exact effect is sometimes contingent on the result of a check or other roll.
...
### Elemental (trait)
Elementals are creatures directly tied to an element and native to the Elemental Planes. Elementals donât need to breathe.
### Elf (trait)
...
### Emanation (area)
### Emotion (trait)
This effect alters a creatureâs emotions. Effects with this trait always have the mental trait as well. Creatures with special training or that have mechanical or artificial intelligence are immune to emotion effects.
### Enchantment (trait)
...
### Environmental (trait)
A hazard with this trait is something dangerous thatâs part of the natural world, such as quicksand or harmful mold.
...
Points that measure a player characterâs progress, accrued during play. Typically a PC gains a new level upon reaching 1, XP.
...
### Fatal (weapon trait)
### Fatigued (condition)
Your defenses are lower, and you canât focus while exploring.
### Fear (trait)
...
### Flat check
A d20 roll that measures pure chance. A flat check canât have any modifiers bonuses, or penalties applied to it.
### Flat-footed (condition)
You take a â2 circumstance penalty to AC.
### Fleeing (condition)
You must run away.
...
### Focus Spell
A type of spell, specific to a class, that can be cast using Focus Points and is automatically heightened to half your level rounded up. Focus spells alwaysÂ have a descriptive term such as âdomain spellâ or âki spell.â
### Focused (trait)
An item with this trait can give you an additional Focus Point. This focus point is separate from your focus pool and doesnât count toward the cap on your focus pool. You can gain this benefit only if you have a focus pool, and there might be restrictions on how the point can be used. You canât gain more than 1 Focus Point per day from focused items.
### Follow the Expert (exploration activity)
Benefit from anotherâs skill proficiency.
### Force (damage type)
### Force (trait)
...
### Fortitude (Fort)
A type of saving throw used to resist diseases, poisons, and otherÂ physical effects. Fortitude modifier = Con modifier + proficiency bonus + otherÂ bonuses + penalties.
### Fortune (trait)
A fortune effect beneficially alters how you roll your dice. You can never have more than one fortune effect alter a single roll. If multiple fortuneÂ effects would apply, you have to pick which to use. If a fortune effect and a misfortune effect would apply to the same roll, the two cancel each other out and you roll normally.
### Free action ([free-action])
An action you can use without spending one of your actions. Free actions with triggers can be used at any time, but they donât use up your 1 reactionÂ per round.
### Free-hand (weapon trait)
### Frequency
An ability that canât be used at will might list a frequency.
### Friendly (condition)
...
### General (trait)
A type of feat that any character can select, regardless of ancestry and class, as long as they meet the prerequisites. You can select a feat with this traitÂ when your class grants a general feat.
### Giant (trait)
Giants are massive humanoid creatures.
### GM (Game Master)
See also Game Master.
### Gnome (trait)
A creature with this trait is a member of the gnome ancestry. Gnomes are small people skilled at magic who seek out new experiences and usually haveÂ low-light vision. An ability with this trait can be used or selected only by gnomes. A weapon with this trait is created and used by gnomes.
### Goblin (trait)
...
### Halfling (trait)
A creature with this trait is a member of the halfling ancestry. TheseÂ small people are friendly wanderers considered to be lucky. An ability with this trait can be used or selected only by halflings. A weapon with this trait is created and used by halflings.
### Hardness
A statistic representing an objectâs durability.
### Haunt (trait)
A hazard with this trait is a spiritual echo, often of someone with a tragicÂ death. Putting a haunt to rest often involves resolving the hauntâs unfinishedÂ business. A haunt that hasnât been properly put to rest always returns after a time.
### Hazard
Hazards are non-creature dangers that adventurers encounter during theirÂ journeys, including environmental hazards, haunts, and traps. Simple hazardsÂ have a one-time effect, but negotiating a complex hazard takes place in encounterÂ mode, wherein the hazard has a specific routine.
### Hazardous terrain (terrain)
You take damage when moving through this terrain.
### Healing (trait)
A healing effect restores a creatureâs body, typically by restoring Hit Points, but sometimes by removing diseases or other debilitating effects.
### Helpful (condition)
...
### Hidden (condition)
A creature knows your location but canât see you.
### Hide [one-action] (skill action)
Make yourself hidden. (Stealth)
### High Jump [two-actions] (skill action)
Jump vertically. (Athletics)
### Hit Points (HP)
A statistic representing the amount of physical harm a creature can take before it falls unconscious or dies. Damage decreases Hit Points on a 1-to-1Â basis, while healing restores Hit Points at the same rate.
### Hostile (condition)
...
An ability with this trait can take a character completely out of the fight or even kill them, and itâs harder to use on a more powerful character. If a spell has the incapacitation trait, any creature of more than twice the spellâs level treats the result of their check to prevent being incapacitated by the spell as one degree of success better, or the result of any check the spellcaster made to incapacitate them as one degree of success worse. If any other effect has the incapacitation trait, a creature of higher level than the item, creature, or hazard generating the effect gains the same benefits.
### Incline (terrain)
You must Climb up an incline.
### Incorporeal (trait)
An incorporeal creature or object has no physical form. It can pass through solid objects, including walls. When inside an object, an incorporeal creature canât perceive, attack, or interact with anything outside the object, and if it starts its turn in an object, it is slowed 1. Corporeal creatures can pass through an incorporeal creature, but they canât end their movement in its space. An incorporeal creature canât attempt Strength-based checks against physical creatures or objectsâonly against incorporeal onesâunless those objects have the ghost touch property rune. Likewise, a corporeal creature canât attempt Strength-based checks against incorporeal creatures or objects. Incorporeal creatures usually have immunity to effects or conditions that require a physical body, like disease, poison, and precision damage. They usually have resistance against all damage (except force damage and damage from Strikes with the ghost touch property rune), with double the resistance against non-magical damage.
...
### Interact [one-action] (basic action)
...
### Invested (trait)
A character can wear only 10 magical items that have the invested trait. None of the magical effects of the item apply if the character hasnât invested it, nor can it be activated, though the character still gains any normal benefitsÂ from wearing the physical item (like a hat keeping rain off their head).
...
### Invisible (condition)
Creatures canât see you.
### Item
An object you carry, hold, or use. Items sometimes grant an item bonus orÂ penalty to certain checks.
### Jousting (weapon trait)
### Key ability
Your key ability is the ability score you use to determine your class DC, as well as your spell attack roll and spell DC if youâre a spellcaster. A key ability for a skill is the ability modifier used for that skill.
### Knocked out
...
### Level
A number that measures somethingâs overall power. A character has a level from 1st to 20th, and other aspects of the game also have levels.
### Lie (skill action)
...
### Lineage (trait)
A feat with this trait indicates a characterâs descendance from a particular type of creature. You can have only one lineage feat. You can select a lineage feat only at 1st level, and you canât retrain into or out of these feats.
### Linguistic (trait)
...
Change someoneâs attitude. (Diplomacy)
### Maneuver in Flight [one-action] (skill action)
Execute a difficult movement while flying. (Acrobatics, trained)
### Manipulate (trait)
You must physically manipulate an item or make gestures to use an action with this trait. Creatures without a suitable appendage canât perform actions with this trait. Manipulate actions often trigger reactions.
...
### Mental (trait)
A mental effect can alter the targetâs mind. It has no effect on an object or a mindless creature.
### Metamagic (trait)
...
A mental effect can alter the targetâs mind. It has no effect on an object or a mindless creature.
### Metamagic (trait)
Actions with the metamagic trait, usually from metamagic feats tweak the properties of your spells. You must use a metamagic action directlyÂ before Casting the Spell you want to alter. If you use any action (including free actions and reactions) other than Cast a Spell directly after, you waste the benefitsÂ of the metamagic action. Any additional effects added by a metamagic action areÂ part of the spellâs effect, not of the metamagic action itself.
### Mindless (trait)
A mindless creature has either programmed or rudimentary mental attributes. Most, if not all, of their mental ability modifiers are â5. They are immune to all mental effects.
### Minion (trait)
Minions are creatures that directly serve another creature. A creature with this trait can use only 2 actions per turn and canât use reactions. Your minion acts on your turn in combat, once per turn, when you spend an action to issue it commands. For an animal companion, you Command an Animal; for a minion thatâs a spell or magic item effect, like a summoned minion, you Sustain a Spell or Sustain an Activation; if not otherwise specified, you issue a verbal command as a single action with the auditory and concentrate traits. If given no commands, minions use no actions except to defend themselves or to escape obvious harm. If left unattended for long enough, typically 1 minute, mindless minions usually donât act, animals follow their instincts, and sapient minions act how they please.
### Misfortune (trait)
...
### Mode of play
The three types of playing the gameâencounters, exploration, and downtimeâeach has a different time scale and degree of rules precision.
### Modifier
A value added to a calculation that can be positive or negative ability modifier
### Monitor (trait)
Creatures that hail from or have a strong connection to the neutrally aligned planes are called monitors. Monitors can survive the basic environmentalÂ effects of planes in the Outer Sphere.
### Monk (trait)
...
### Multiclass (trait)
Archetypes with the multiclass trait represent diversifying your training into another classâs specialties. You canât select a multiclass archetypeâsÂ dedication feat if you are a member of the class of the same name.
### Multiple attack Penalty
You take this penalty on all attacks after the first on your turn. This is a â5 penalty on your second attack and â10 on all subsequent attacksÂ (or â4 and â8 if your weapon or unarmed attack has the agile trait).
### Mutagen (trait)
An elixir with the mutagen trait temporarily transmogrifies the subjectâs body and alters its mind. A mutagen always conveys one or moreÂ beneficial effects paired with one or more detrimental effects. Mutagens areÂ polymorph effects, meaning you can benefit from only one at a time.
### Narrow Surface (terrain)
You must Balance to cross a narrow surface
### Natural 1, Natural 20
When you roll a d20 and the number on the die is a 1, decrease your degree of success by one step. When the number is a 20, increase the degreeÂ of success by one step.
### Nature (skill)
...
Attacks with this weapon are nonlethal, and are used to knock creatures unconscious instead of kill them. You can use a nonlethal weapon to make a lethal attack with a â2 circumstance penalty.
...
Oozes are creatures with simple anatomies. They tend to have low mental ability scores and immunity to mental effects and precision damage.
### Open (trait)
These maneuvers work only as the first salvo on your turn. You can use an open only if you havenât used an action with the attack or open trait yet this turn.
### Oracle (class) multiclass archetype revelation spells â oracle (trait)
...
### Orc (trait)
A creature with this trait is a member of the orc ancestry. TheseÂ green-skinned people tend to have darkvision. An ability with this trait can be usedÂ or selected only by orcs. An item with this trait is created and used by orcs.
### Orichalcum (material)
### Paladin
...
### Paralyzed (condition)
You canât move or fully defend yourself.
### Parry (weapon trait)
...
### Perception
A statistic measuring your ability to notice hidden objects or unusualÂ situations. Perception is usually used for initiative rolls. Perception modifier = Wis modifier + proficiency bonus + other bonuses + penalties.
...
A character created and controlled by a playerÂ other than the GM.
### Point Out [one-action] (specialty basic action)
Indicate an undetected creatureâs location.
### Poison (damage type)
### Poison (trait)
...
### Polymorph (trait)
These effects transform the target into a new form. A target canâtÂ be under the effect of more than one polymorph effect at a time. If it comes under the effect of a second polymorph effect, the second polymorph effect attempts to counteract the first. If it succeeds, it takes effect, and if it fails, the spell hasÂ no effect on that target. Any Strikes specifically granted by a polymorph effect are magical. Unless otherwise stated, polymorph spells donât allow the target to take on the appearance of a specific individual creature, but rather just a genericÂ creature of a general type or ancestry. If you take on a battle form with a polymorph spell, the special statistics canÂ be adjusted only by circumstance bonuses, status bonuses, and penalties. UnlessÂ otherwise noted, the battle form prevents you from casting spells, speaking and using most manipulate actions that require hands. (If thereâs doubt aboutÂ whether you can use an action, the GM decides.) Your gear is absorbed into you; the constant abilities of your gear still function, but you canât activate any items.
### Positive (trait)
...
### Possession (trait)
Effects with this trait allow a creature to project its mind and spirit into a target. A creature immune to mental effects canât use a possession effect.Â While possessing a target, a possessorâs true body is unconscious (and canât wakeÂ up normally), unless the possession effect allows the creature to physically enter the target. Whenever the target takes damage, the possessor takes half that amount of damage as mental damage. A possessor loses the benefits of any of its active spells or abilities that affect its physical body, though it gains the benefits of the targetâs active spells and abilities that affect their body. A possessor can use any of the targetâs abilities that are purely physical, and it canât use any of its own abilities except spells and purely mental abilities. The GM decides whether an ability is purely physical orÂ purely mental. A possessor uses the targetâs attack modifier, AC, Fortitude saveÂ Reflex save, Perception, and physical skills, and its own Will save, mental skillsÂ spell attack roll, and spell DC; benefits of invested items apply where relevantÂ (the possessorâs invested items apply when using its own values, and the targetâs invested items apply when using the targetâs values). A possessor gains no benefitÂ from casting spells that normally affect only the caster, since it isnât in its own body. The possessor must use its own actions to make the possessed creature act. If a possessor reaches 0 Hit Points through any combination of damage to its true body and mental damage from the possession, it is knocked out as normal and the possession immediately ends. If the target reaches 0 Hit Points first, the possessor can either fall unconscious with the body and continue the possessionÂ or end the effect as a free action and return to its body. If the target dies, the possession ends immediately and the possessor is stunned for 1 minute.
### Potion (trait)
A potion is a magical liquid activated when you drink it.
### Pp (platinum piece)
### Precious (trait)
Valuable materials with special properties have the precious trait.Â They can be substituted for base materials when you Craft items.
### Precise sense
A sense that can make creatures observed, such as human sight.
### Precision (damage type)
A type of damage that increases the attackâs listed damage, using the same damage type, rather than adding a separate amount.
### Prediction (trait)
Effects with this trait determine what is likely to happen in the nearÂ future. Most predictions are divinations.
### Prerequisite
Many feats and other abilities can be taken only if you meet theirÂ prerequisites. Prerequisites are often feats or proficiency ranks.
### Press (trait)
Actions with this trait allow you to follow up earlier attacks. An actionÂ with the press trait can be used only if you are currently affected by a multiple attack penalty. Some actions with the press trait also grant an effect on a failure.Â The effects that are added on a failure donât apply on a critical failure. If your press action succeeds, you can choose to apply the failure effect instead. (For example you may wish to do this when an attack deals no damage due to resistance.)Â Because a press action requires a multiple attack penalty, you canât use one when itâs not your turn, even if you use the Ready activity.
### Price
The amount of currency it usually costs to purchase an item.
### Primal (trait)
This magic comes from the primal tradition, connecting to the naturalÂ world and instinct. Anything with this trait is magical.
### Proficiency
A measure of a characterâs aptitude at a specific task or quality, withÂ five ranks: untrained, trained, expert, master, and legendary. Proficiency gives a proficiency bonus. Being untrained adds a +0 bonus. Being trained, expert, master, or legendary adds your level plus 2, 4, 6, or 8, respectively.
### Prone (condition)
Youâre lying on the ground and easier to attack.
### Propulsive (weapon trait)
### Quickened (condition)
You get an extra action each turn.
### Rage (trait)
### Raise a Shield [one-action] (specialty basic action)
Gain your shieldâs bonus to AC.
### Ranger (class)
### Ranger (trait)
...
### Rarity
How often something is encountered in the game world. The rarities are common,Â uncommon, rare, and unique. Anything that doesnât list a rarity is common.
### Reach (weapon trait)
### Reaction ([reaction])
An action you can use even if itâs not your turn. You can use 1 reactionÂ per round.
### Ready [two-actions] (basic action)
Prepare an action to use when itâs not your turn.
### Recall Knowledge [one-action] (skill action)
...
Let go of something youâre holding.
### Religion (skill)
...
### Rest
Characters recover HP (normally Con modifier Ã level) and resources with 8Â hours of sleep.
### Restrained (condition)
You are bound by restraints or a grappling creature.
### Retrain
You can retrain during downtime to change character choices.
### Revelation Spell
Revelation spells are focus spells cast by oracles. Revelation spells have the cursebound trait, which means they progress the severity of your oracular curse when cast, and you canât cast them if you donât have an oracular curse.
### Revelation (trait)
Effects with this trait see things as they truly are.
### Ritual
A type of spell that takes downtime to cast and doesnât use spell slots.
### Rogue (trait)
This indicates abilities from the rogue class.
### Roleplaying
Describing a characterâs actions, often while acting from the perspectiveÂ of the character.
### Roleplaying game (RPG)
An interactive story where one player, the Game MasterÂ (GM), sets the scene and presents challenges, while other players take the roles ofÂ player characters (PCs) and attempt to overcome those challenges.
### Roll
Any time you roll the dice, youâre making a roll. The most common type of roll is a check (comparing a d20 plus modifiers against a DC).
### Round
...
An effect with this trait delivers a poison or deals poison damage. An item with this trait is poisonous and might cause an affliction.
### Polymorph (trait)
These effects transform the target into a new form. A target canât be under the effect of more than one polymorph effect at a time. If it comes under the effect of a second polymorph effect, the second polymorph effect attempts to counteract the first. If it succeeds, it takes effect, and if it fails, the spell has no effect on that target. Any Strikes specifically granted by a polymorph effect are magical. Unless otherwise stated, polymorph spells donât allow the target to take on the appearance of a specific individual creature, but rather just a generic creature of a type or ancestry. If you take on a battle form with a polymorph spell, the special statistics can be adjusted only by circumstance bonuses, status bonuses, and penalties. Unless otherwise noted, the battle form prevents you from casting spells, speaking, and using most manipulate actions that require hands. (If thereâs doubt about whether you can use an action, the GM decides.) Your gear is absorbed into you; the constant abilities of your gear still function, but you canât activate any items.
### Positive (trait)
Effects with this trait heal living creatures with positive energy, deal positive energy damage to undead, or manipulate positive energy.
### Potion (trait)
A potion is a magical liquid activated when you drink it.
...
### Press (trait)
Actions with this trait allow you to follow up earlier attacks. An action with the press trait can be used only if you are currently affected by a multiple attack penalty. Some actions with the press trait also grant an effect on a failure. The effects that are added on a failure donât apply on a critical failure. If your press action succeeds, you can choose to apply the failure effect instead. (For example, you may wish to do this when an attack deals no damage due to resistance.) Because a press action requires a multiple attack penalty, you canât use one when itâs not your turn, even if you use the Ready activity.
### Primal (trait)
...
This weapon can be used to Trip with the Athletics skill at a distance up to the weaponâs first range increment. The skill check takes a â2 circumstance penalty. You can add the weaponâs item bonus to attack rolls as a bonus to the check. As with using a melee weapon to trip, a ranged trip doesnât deal any damage when used to Trip. This trait usually appears only on a thrown weapon.
### Rare (trait)
This rarity indicates that a rules element is very difficult to find in the game world. A rare feat, spell, item or the like is available to players only if the GM decides to include it in the game, typically through discovery during play.
### Ratfolk (trait)
...
### Saving throw (check)
A roll made to avoid or mitigate a dangerous effect. You roll a save automatically, with no action or a reaction. The character who isnât acting rolls the d20 for a saving throw, and the creature who is acting provides the DC. There are three types of saves: Fortitude, Reflex, and Will.
### Scent (sense)
...
### Shove (weapon trait)
You can use this weapon to Shove with the Athletics skill even if you donât have a free hand. This uses the weaponâs reach (if different from your own) and adds the weaponâs item bonus to attack rolls as an item bonus to the Athletics check. If you critically fail a check to Shove using the weapon, you can drop the weapon to take the effects of a failure instead of a critical failure.
### Shove [one-action] (skill action)
Push a creature. (Athletics)
### Sickened (condition)
Youâre sick to your stomach.
### Silver (material)
### Silver piece (sp)
### Single action ([one-action])
...
### Skill
A statistic representing the ability to perform certain tasks that require instruction or practice. Skill modifier = modifier of the skillâs key ability score + proficiency bonus + other bonuses + penalties.
### Skill feat
...
### Speed
A measure of the distance a character can move using a single actionÂ measured in feet. See also movement.
### Spell
...
### Spell Level
Spells range in level from 1â10.
### Spell Repertoire
...
### Splash (trait)
When you use a thrown weapon with the splash trait, you donât add your Strength modifier to the damage roll. If an attack with a splash weapon fails, succeeds, or critically succeeds, all creatures within 5 feet of the target (including the target) take the listed splash damage. On a failure (but not a critical failure), the target of the attack still takes the splash damage. Add splash damage together with the initial damage against the target before applying the targetâs weaknesses or resistances. You donât multiply splash damage on a critical hit.
...
### Stance (trait)
A stance is a general combat strategy that you enter by using an action with the stance trait, and that you remain in for some time. A stance lastsÂ until you get knocked out, until its requirements (if any) are violated, until the encounter ends, or until you enter a new stance, whichever comes first. After youÂ use an action with the stance trait, you canât use another one for 1 round. You canÂ enter or be in a stance only in encounter mode.
...
Pilfer an object in someone elseâs possession. (Thievery)
### Stealth (skill)
...
### Structure (trait)
An item with the structure trait creates a magical building or other structure when activated. The item must be activated on a plot of land free of other structures. The structure adapts to the natural terrain, adopting the structural requirements for being built there. The structure adjusts around small features such as ponds or spires of rock, but it canât be created on water or other nonsolid surfaces. If activated on snow, sand dunes, or other soft surfaces with a solid surface underneath, the structureâs foundation (if any) reaches the solid ground. If an item with this trait is activated on a solid but unstable surface, such as a swamp or an area plagued by tremors, roll a DC 3 flat check each day; on a failure, the structure begins to sink or collapse. The structure doesnât harm creatures within the area when it appears, and it canât be created within a crowd or in a densely populated area. Any creature inadvertently caught inside the structure when the item is activated ends up unharmed inside the complete structure and always has a clear path of escape. A creature inside the structure when the activation ends isnât harmed, and it lands harmlessly on the ground if it was on an upper level of the structure.
### Stunned (condition)
You canât act for a number of actions or an amount of time.
### Stupefied (condition)
Your canât access your full mental faculties, and you have trouble casting spells.
### Subsist (skill action)
Find food and shelter for free. (Society or Survival)
### Success
A result on a check that equals or exceeds the DC. Exceeding the DC by or more is even betterâa critical success. If a stat block has no success entry, that means there is no effect on a success.
### Summoned (trait)
A creature called by a conjuration spell or effect gains the summoned trait. A summoned creature canât summon other creatures, create things of value, or cast spells that require a cost. It has the minion trait. If it tries to cast a spell of equal or higher level than the spell that summoned it, it overpowers the summoning magic, causing the summoned creatureâs spell to fail and the summon spell to end. Otherwise, the summoned creature uses the standard abilities for a creature of its kind. It generally attacks your enemies to the best of its abilities. If you can communicate with it, you can attempt to command it, but the GM determines the degree to which it follows your commands. Immediately when you finish Casting the Spell, the summoned creature uses its 2 actions for that turn. Summoned creatures can be banished by various spells and effects. They are automatically banished if reduced to 0 Hit Points or if the spell that called them ends.
A summoned creature can’t control any spawn or other creatures generated from it, and such creatures return to their unaltered state (usually a corpse in the case of spawn) once the summoned creature is gone. If it’s unclear what this state would be, the GM decides.Â
### Support [one-action]
...
### Take Cover [one-action] (basic action)
...
Teleportation effects allow you to instantaneously move fromÂ one point in space to another. Teleportation does not usually trigger reactionsÂ based on movement.
### Teleportation (trait)
...
### Temporary Immunity
An effect that makes a creature temporarily immune lets thatÂ creature avoid new effects of the same name, but it doesnât end anything causedÂ by the effect that gave the temporary immunity.
### Temporary Items
...
### Track (skill action)
Follow a creatureâs tracks. (Survival)
### Tradition
...
### Transmutation (trait)
Effects and magic items with this trait are associated with the transmutation school of magic, typically changing somethingâs form.
### Trap (trait)
A hazard or item with this trait is constructed to hinder interlopers. â travel Speed treasure See also item.
...
Move through someoneâs space. (Acrobatics) turn During a round in an encounter, each creature takes a single turn. A creature typically uses up to 3 actions during its turn.
### Two-hand (weapon trait)
This weapon can be wielded with two hands. Doing so changes its weapon damage die to the indicated value. This change applies to all the weaponâs damage dice, such as those from striking runes.
### Unarmed attack
...
Youâre asleep or knocked out.
### Undead (trait)
Once living, these creatures were infused after death with negative energy and soul-corrupting evil magic. When reduced to 0 Hit Points, an undead creature is destroyed. Undead creatures are damaged by positive energy, are healed by negative energy, and donât benefit from healing effects.
### Undetected (condition)
A creature doesnât know your precise location.
### Uneven ground (terrain)
You must Balance or fall when crossing uneven ground.
### Unfriendly (condition)
An NPC with this condition doesnât like you.
### Unique (trait)
A rules element with this trait is one-of-a-kind.
### Unlimited
A spell with this duration lasts indefinitely.
### Unnoticed (condition)
A creature is entirely unaware youâre present.
...
### Vague sense
A sense that can detect an unnoticed creature but not determine its position, such as a humanâs sense of smell.
### Versatile Heritage Hex (trait)
A hex is a short-term effect generated on the fly from your patronâs magic, requiring your familiar to draw from your patron. As such, you can cast only one spell with the hex trait each turn; attempts to cast a second hex spell on the same turn fail, and the spellcasting actions are lost. hostile action a hostile action is one that can harm or damage another creature, whether directly or indirectly, but not one that a creature is unaware could cause harm.
### Virulent (trait)
Afflictions with the virulent trait are harder to remove. You must succeed at two consecutive saves to reduce a virulent afflictionâs stage by 1. A critical success reduces a virulent afflictionâs stage by only 1 instead of by 2.
### Visual (trait)
...
### Wealth
A characterâs wealth by level.
### Will
...
### Wounded (condition)
Youâve returned from the brink of death but remain at risk.
